/*
  ...
  Règle : le client ne choisit jamais utilisateur_id.
*/
...
export const getBudgets = asyncHandler(async (request, response) => {
  ...
  const { categorieId, mois, annee, limite, page, offset } = validation.donnees
  const utilisateurId = request.utilisateur.utilisateurId

  const resultat = await findAllBudgets(utilisateurId, categorieId, mois, annee, limite, offset)
...
export const getBudgetById = asyncHandler(async (request, response) => {
  ...
  const budget = await findBudgetById(validation.donnees.id, request.utilisateur.utilisateurId)
...
// Try/catch conservé : 23503 (catégorie inexistante) et 23505 (doublon catégorie+période).
export const postBudget = asyncHandler(async (request, response) => {
  ...
  try {
    const nouveauBudget = await createBudget(request.utilisateur.utilisateurId, validation.donnees)
    ...
    if (estErreurDoublon(error)) {
      throw new ErreurHTTP(409, "Un budget existe déjà pour cette catégorie et cette période")
    }
    throw error
  }
})

export const putBudget = asyncHandler(async (request, response) => {
...
export const deleteBudgetById = asyncHandler(async (request, response) => {
  const validation = validerIdBudget(request.params.id)
  ...
  const budgetSupprime = await deleteBudget(validation.donnees.id, request.utilisateur.utilisateurId)

  if (!budgetSupprime) {
    throw new ErreurHTTP(404, "Budget introuvable")
  }

  response.json({ message: "Budget supprimé", budget: budgetSupprime })
})